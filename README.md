...
面向中国徒步爱好者的一站式行前决策工具：支持先发现有公开来源的候选路线，再围绕用户已有或下载得到的 KML 轨迹完成路线量化分析、天气与周边信息搜集、交通方案整理，并通过 AI 生成可执行的行程、装备与安全攻略。
...
- “发现路线”模块可从本地路线知识库和百炼联网搜索中筛选有证据来源的路线名称，并提供路线库 CRUD 与导入审核工作台；
...
- 当前自动化测试共 99 项，覆盖路线解析、路线分析、路线发现、知识库管理、Agent 分支、服务降级和主要 API。

## 产品使用流程与页面展示

下面按照用户实际使用顺序，展示从寻找路线到获得完整徒步攻略的主要流程。

### 1. 首页与发现路线入口

用户进入网站后，可以上传新的 KML 轨迹，也可以从左侧历史列表恢复、收藏或清理此前生成的攻略。页面同时显示后端服务状态；如果用户还没有明确路线，可以先进入“发现路线”模块筛选有公开来源的候选名称，再回到 KML 分析链路。

![Trailer 首页与发现路线入口](docs/screenshots/01-home-and-route-discovery.png)

### 2. 发现路线与路线库管理

“发现路线”只负责给出可追溯的路线名称，不直接生成轨迹。系统先检索本地路线知识库；候选不足时再调用百炼 Responses API 联网搜索，并用公开来源、高德地点核验或双来源门槛过滤结果。页面右侧提供“路线库管理”入口，可新增、编辑、归档路线知识卡，也可以从 URL 或正文导入候选并人工审核合并。

![路线库管理表格](docs/screenshots/02-route-knowledge-manager.png)

![资料导入审核工作台](docs/screenshots/03-route-knowledge-import.png)
...
- **证据约束的路线发现**：根据用户自然语言需求解析地区、天数、强度和偏好，优先检索本地路线知识库，不足时联网补充；只展示有公开来源且通过地点或双来源门槛的候选路线名称。
- **路线知识库治理**：基于 SQLite + FTS5 管理路线知识卡，支持新增、编辑、归档、硬删除、精选路线、来源记录、导入候选审核和合并。
...
- DashScope Responses API + web search（路线发现）
...
LangGraph HikingGuideAgent
  ...
  plan_tools → collect_weather → analyze_route
       ...
       → compose_guide → collect_reference
             ...
             ▼
Provider / Service 层
  ├── 路线：KML / 知识库检索 / 百炼联网搜索 / 高德地点核验 / OpenRouteService
  ├── 天气：高德 + Open-Meteo
  ├── 行前信息：高德 POI + 静态兜底
...
1. 用户用自然语言描述地区、天数、体能、景观或交通偏好。
2. LLM 只负责结构化需求字段，不直接编造路线事实；字段会标记为明确、默认或未知。
3. 系统优先检索本地路线知识库；候选不足时再调用百炼联网搜索补充公开网页中的真实路线名称。
4. 候选必须带公开来源，并通过高德地点核验或至少两个独立来源；不满足证据门槛时宁可少返回。
5. 用户选中路线名称后，需要到外部来源核对并获取 KML，再进入路线分析和攻略生成链路。
...
- **问题**：如果用户还没有 KML，直接让模型“推荐路线”很容易生成听起来合理但无法核验的路线名称。
- **方案**：把路线发现拆成需求解析、知识库检索、联网补充、地点核验、证据门槛和排序；模型只做解析和受约束搜索，最终展示由代码过滤。
- **设计原因**：发现路线是进入主链路前的辅助入口，不应替代轨迹文件和路线事实分析。
- **效果**：系统可以给出少量有来源的候选名称和核验项，也会在证据不足时明确返回空结果，而不是用模拟路线补齐。

### 亮点二：基于 LangGraph 的可降级工作流
...
| `GET` | `/route-knowledge-manager` | 打开路线库管理深链页面 |
| `POST` | `/api/v1/route-recommendations` | 根据自然语言需求返回有证据的候选路线名称 |
| `POST` | `/api/v1/route-recommendations/stream` | 以 SSE 返回路线发现阶段和最终推荐 |
| `GET` | `/api/v1/route-recommendations/featured` | 从路线知识库返回全国或区域精选 |
| `GET` | `/api/v1/route-knowledge` | 查询路线知识库条目 |
| `POST` | `/api/v1/route-knowledge` | 新增路线知识卡 |
| `GET` | `/api/v1/route-knowledge/{route_id}` | 获取路线知识卡详情 |
| `PUT` | `/api/v1/route-knowledge/{route_id}` | 更新路线知识卡 |
| `DELETE` | `/api/v1/route-knowledge/{route_id}` | 归档或永久删除路线知识卡 |
| `POST` | `/api/v1/route-knowledge/import-jobs` | 从 URL 或正文创建路线导入审核任务 |
| `GET` | `/api/v1/route-knowledge/import-jobs/{job_id}` | 查看导入候选与匹配结果 |
| `POST` | `/api/v1/route-knowledge/import-jobs/{job_id}/apply` | 应用导入任务，新增、合并或忽略候选 |
...
├── data/                    # 运行时路线知识库 SQLite 文件，首次使用自动生成
├── tests/                   # pytest 自动化测试
...
- “发现路线”只推荐有来源的路线名称，不生成可导航轨迹；正式分析仍以用户上传 KML 为主。
